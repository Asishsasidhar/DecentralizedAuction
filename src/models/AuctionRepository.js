...
export class AuctionRepository {
  ...
  async watchIfBidSuccess(cb) {
    const currentBlock = await this.getCurrentBlock();
    const eventWatcher = this.contractInstance.BidSuccess(
      {},
      { fromBlock: currentBlock - 1, toBlock: "latest" }
    );
    eventWatcher.watch(cb);
  ...
  async watchIfCanceled(cb) {
    const currentBlock = await this.getCurrentBlock();
    const eventWatcher = this.contractInstance.AuctionCanceled(
      {},
      { fromBlock: currentBlock - 1, toBlock: "latest" }
    );
    eventWatcher.watch(cb);
  ...
  async watchIfFinalized(cb) {
    const currentBlock = await this.getCurrentBlock();
    const eventWatcher = this.contractInstance.AuctionFinalized(
      {},
      { fromBlock: currentBlock - 1, toBlock: "latest" }
    );
    eventWatcher.watch(cb);
  ...
  getCurrentBid(auctionId) {
    return new Promise(async (resolve, reject) => {
      try {
        this.contractInstance.getCurrentBid(
          auctionId,
  ...
  getBidCount(auctionId) {
    return new Promise(async (resolve, reject) => {
      try {
        this.contractInstance.getBidsCount(
          auctionId,
          { from: this.account, gas: this.gas },
          (err, transaction) => {
            if (!err) resolve(transaction);
    ...
    return new Promise(async (resolve, reject) => {
      try {
        this.contractInstance.methods
          .getCount()
          .call({ from: this.account, gas: this.gas }, (err, transaction) => {
            if (!err) resolve(transaction);
            reject(err);
          });
      } catch (e) {
  ...
  bid(auctionId, price) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        this.contractInstance.bidOnAuction(
          auctionId,
          {
            from: this.account,
            gas: this.gas,
  ...
  cancel(auctionId) {
    return new Promise(async (resolve, reject) => {
      try {
        this.contractInstance.cancelAuction(
          auctionId,
          { from: this.account, gas: this.gas },
          (err, transaction) => {
            if (!err) resolve(transaction);
  ...
  finalize(auctionId) {
    return new Promise(async (resolve, reject) => {
      try {
        this.contractInstance.finalizeAuction(
          auctionId,
          { from: this.account, gas: this.gas },
          (err, transaction) => {
            if (!err) resolve(transaction);
  ...
  findById(auctionId) {
    return new Promise(async (resolve, reject) => {
      try {
        this.contractInstance.getAuctionById(
          auctionId,
          { from: this.account, gas: this.gas },
          (err, transaction) => {
            if (!err) resolve(transaction);
