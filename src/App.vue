                ...
                <v-card class="mb-5">
                  <v-layout xs12 sm12 md12 row wrap>
                    ...
                    >
                      ...
                      <v-btn
                        v-show="!createAssetSuccess"
                        :disabled="deed.deedURI.length == 0 || creatingAsset"
                        color="teal"
                        outline
                        @click.native="registerDeed()"
        ...
        <v-layout style="padding:10px;" wrap row>
          ...
          <v-flex xs12 sm4 md4>
            <div v-show="getNetworkId != ''">
              <v-icon color="teal">check_circle</v-icon> Connected to network:
              {{ getNetworkId }}
            </div>
            <div v-show="getNetworkId == ''">
          ...
          <v-flex xs12 sm4 md4>
            <div v-show="getWeb3DefaultAccount != ''">
              <v-icon color="teal">check_circle</v-icon> Account:
  ...
  methods: {
    async transferTo() {
      try {
        ...
        const res = await this.$deedRepoInstance.transferTo(
          this.$config.AUCTIONREPOSITORY_ADDRESS,
          this.selectedDeed
        );
    ...
    async uploadAndCreateAuction() {
      try {
        ...
        const transaction = await this.$auctionRepoInstance.create(
          this.auction.deedId,
    ...
    async registerDeed() {
      try {
        ...
        this.$deedRepoInstance.watchIfCreated((error, result) => {
          ...
          if (!error) this.createAssetSuccess = true;
          else
    ...
    showAuction() {
      // get random deedid
      this.deed.deedId = new web3.utils.BN(
        `${this.$root.$data.globalState.getRandomInt(
          123456789,
          999999999
        )}${this.$root.$data.globalState.getRandomInt(123456789, 999999999)}`
      );
      this.dialog = true;
    },
